...
public class Day01 implements Day<Integer[]> {
    ...
    public String part1(Integer[] input) {
        return Arrays.stream(input)
                .flatMap(a -> Arrays.stream(input).map(b -> Tuple.of(a, b)))
                .filter(tuple -> tuple._1 + tuple._2 == 2020)
                .findFirst()
    ...
    public String part2(Integer[] input) {
        return Arrays.stream(input)
                .flatMap(a -> Arrays.stream(input).map(b -> Tuple.of(a, b)))
                .flatMap(tuple -> Arrays.stream(input).map(c -> Tuple.of(tuple._1, tuple._2, c)))
                .filter(tuple -> tuple._1 + tuple._2 + tuple._3 == 2020)
                .findFirst()
