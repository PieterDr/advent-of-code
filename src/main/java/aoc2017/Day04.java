...
public class Day04 {
    ...
    public static void main(String[] args) {
        ...
        List<String> passPhraseList = asList(input.split("\n"));
        int result = passPhraseList.stream()
                .map(phrase -> asList(phrase.split(" ")))
                .map(Day04::sortLettersOfEachWordAlphabetically)
    ...
    private static List<String> sortLettersOfEachWordAlphabetically(List<String> phrase) {
        return phrase.stream()
                .map(String::toCharArray)
                .map(Day04::sortAlphabetically)
                .map(String::new)
                .collect(toList());
    }

    private static char[] sortAlphabetically(char[] letters) {
        List<Character> chars = new ArrayList<>();
        for (char letter: letters) {
            chars.add(letter);
        }
        List<Character> alphabeticLetters = chars.stream().sorted(Comparator.naturalOrder()).collect(toList());
        for(int i = 0; i < letters.length; i++) {
            letters[i] = alphabeticLetters.get(i);
        }
        return letters;
    }

    private static boolean isValid(List<String> phrase) {
        if (phrase.size() == 1) {
            return true;
        }
        String x = phrase.get(0);
        List<String> xs = phrase.subList(1, phrase.size());
        return !xs.contains(x) && isValid(xs);
    }
}
